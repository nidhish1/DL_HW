...
def gradient_check(model, loss_fn, x: np.ndarray, y: np.ndarray, epsilon: float = 1e-5) -> float:
    ...
    """
    # 1. Analytical gradients via forward + backward
    output = model.forward(x)
    loss_fn.forward(output, y)
    ...
    # 2. For each parameter, perturb each element and compute numerical gradient
    for param, anal_grad in zip(parameters, analytical_grads):
        ...
        while not it.finished:
            ...
            numerical_ij = (loss_plus - loss_minus) / (2.0 * epsilon)
            anal_ij = anal_grad[idx]
            diff = np.abs(anal_ij - numerical_ij)
            denom = np.abs(anal_ij) + np.abs(numerical_ij) + 1e-12
            rel_err = diff / denom
